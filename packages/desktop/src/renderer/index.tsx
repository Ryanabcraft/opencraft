...
import "./styles.css"
import { useTheme } from "@OPENCRAFT-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OPENCRAFT_CHANNEL === "prod" &&
...
const deepLinkEvent = "OPENCRAFT:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__OPENCRAFT__ ??= {}
  const pending = window.__OPENCRAFT__.deepLinks ?? []
  window.__OPENCRAFT__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("OPENCRAFT.global.dat").getItem("language")
