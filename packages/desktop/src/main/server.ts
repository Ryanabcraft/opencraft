...
const SIDECAR_SERVICE_NAME = "OPENCRAFT server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    OPENCRAFT_EXPERIMENTAL_ICON_DISCOVERY: "true",
    OPENCRAFT_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`OPENCRAFT:${password}`).toString("base64")
