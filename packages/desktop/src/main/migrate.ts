...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.OPENCRAFT.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `OPENCRAFT.settings.dat` is special: it maps to the `OPENCRAFT.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"OPENCRAFT.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // OPENCRAFT.settings.dat → the electron settings store ("OPENCRAFT.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "OPENCRAFT.global.dat").
  const storeName = filename === "OPENCRAFT.settings.dat" ? "OPENCRAFT.settings" : filename
