...
import { createOPENCRAFT, type ToolPart } from "@OPENCRAFT-ai/sdk"
...
console.log("🚀 Starting OPENCRAFT server...")
const OPENCRAFT = await createOPENCRAFT({
  port: 0,
})
console.log("✅ OPENCRAFT server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await OPENCRAFT.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new OPENCRAFT session...")
    const { client, server } = OPENCRAFT
    ...
    console.log("✅ Created OPENCRAFT session:", createResult.data.id)
  ...
  console.log("📝 Sending to OPENCRAFT:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 OPENCRAFT response:", JSON.stringify(result, null, 2))
