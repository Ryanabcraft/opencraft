...
import { type Session } from "@OPENCRAFT-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("OPENCRAFT://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("OPENCRAFT://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("OPENCRAFT://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("OPENCRAFT://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("OPENCRAFT://open-project")).toBeUndefined()
    expect(parseDeepLink("OPENCRAFT://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "OPENCRAFT://open-project?directory=/a",
      "OPENCRAFT://other?directory=/b",
      "OPENCRAFT://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("OPENCRAFT://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("OPENCRAFT://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("OPENCRAFT://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("OPENCRAFT://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "OPENCRAFT://new-session?directory=/a",
      "OPENCRAFT://open-project?directory=/b",
      "OPENCRAFT://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCRAFT__: {
        deepLinks: ["OPENCRAFT://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCRAFT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["OPENCRAFT://open-project?directory=/a"])
