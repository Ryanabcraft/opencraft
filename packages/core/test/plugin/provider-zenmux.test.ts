...
import { ProviderPlugins } from "@OPENCRAFT-ai/core/plugin/provider"
import { ZenmuxPlugin } from "@OPENCRAFT-ai/core/plugin/provider/zenmux"
...
describe("ZenmuxPlugin", () => {
  ...
  it.effect("applies the exact legacy Zenmux headers", () =>
    Effect.gen(function* () {
      ...
      expect(result.provider.options.headers).toEqual({ "HTTP-Referer": "https://OPENCRAFT.ai/", "X-Title": "OPENCRAFT" })
