...
import { AuthV2 } from "@OPENCRAFT-ai/core/auth"
import { ModelV2 } from "@OPENCRAFT-ai/core/model"
import { PluginV2 } from "@OPENCRAFT-ai/core/plugin"
import { AuthPlugin } from "@OPENCRAFT-ai/core/plugin/auth"
import { CloudflareWorkersAIPlugin } from "@OPENCRAFT-ai/core/plugin/provider/cloudflare-workers-ai"
  ...
  it.effect("uses env API key over auth or configured API key and keeps the Cloudflare User-Agent", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "env-key" }, () =>
      Effect.gen(function* () {
        ...
        expect(headers["user-agent"]).toMatch(/^OPENCRAFT\/.* cloudflare-workers-ai \(.+\) ai-sdk\/openai-compatible\//)
      }),
    ),
  )
