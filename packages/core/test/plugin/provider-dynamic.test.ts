...
import { AISDK } from "@OPENCRAFT-ai/core/aisdk"
import { ModelV2 } from "@OPENCRAFT-ai/core/model"
import { PluginV2 } from "@OPENCRAFT-ai/core/plugin"
import { DynamicProviderPlugin } from "@OPENCRAFT-ai/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "OPENCRAFT-provider-dynamic-"))
