...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@OPENCRAFT/acme")).toBe("@OPENCRAFT/acme")
    expect(Npm.sanitize("@OPENCRAFT/acme@1.0.0")).toBe("@OPENCRAFT/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/OPENCRAFT/acme.git"
    const expected = win ? "acme@git+https_//github.com/OPENCRAFT/acme.git" : spec
