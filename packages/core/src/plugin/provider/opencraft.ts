...
export const OPENCRAFTPlugin = PluginV2.define({
  id: PluginV2.ID.make("OPENCRAFT"),
  effect: Effect.gen(function* () {
    let hasKey = false
    return {
      "provider.update": Effect.fn(function* (evt) {
        if (evt.provider.id !== ProviderV2.ID.OPENCRAFT) return
        hasKey = Boolean(
          process.env.OPENCRAFT_API_KEY ||
      ...
      "model.update": Effect.fn(function* (evt) {
        if (evt.model.providerID !== ProviderV2.ID.OPENCRAFT) return
        if (hasKey) return
