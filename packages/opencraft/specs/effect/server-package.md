...
Practical reference for a future `packages/server` split after the OPENCRAFT
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/OPENCRAFT`.
...
Until enough shared service code lives outside `packages/OPENCRAFT`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/OPENCRAFT`

It should not import `packages/OPENCRAFT` services while `packages/OPENCRAFT`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on OPENCRAFT-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/OPENCRAFT` implementation details.
