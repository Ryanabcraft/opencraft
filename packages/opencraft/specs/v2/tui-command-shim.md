...
Delete `packages/OPENCRAFT/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/OPENCRAFT/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/OPENCRAFT/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/OPENCRAFT`
- TUI plugin loader tests in `packages/OPENCRAFT` if runtime plugin API wiring changed
