...
Replace runtime Bun shell template-tag usage in `packages/OPENCRAFT/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/OPENCRAFT/src` is removed except:
