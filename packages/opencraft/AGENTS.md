...
- Running `bun dev` from `packages/OPENCRAFT` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s OPENCRAFT-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt OPENCRAFT-dev`.
- Stop the session explicitly when done: `tmux kill-session -t OPENCRAFT-dev`.
