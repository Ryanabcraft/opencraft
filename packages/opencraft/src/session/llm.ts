...
import type { LLMEvent } from "@OPENCRAFT-ai/llm"
import { LLMClient, RequestExecutor } from "@OPENCRAFT-ai/llm/route"
import type { LLMClientService } from "@OPENCRAFT-ai/llm/route"
...
export class Service extends Context.Service<Service, Interface>()("@OPENCRAFT/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via OPENCRAFT's tool system
      ...
      const OPENCRAFTProjectID = input.model.providerID.startsWith("OPENCRAFT")
        ? (yield* InstanceState.context).project.id
        : undefined

      const requestHeaders = {
        ...(input.model.providerID.startsWith("OPENCRAFT")
          ? {
              ...(OPENCRAFTProjectID ? { "x-OPENCRAFT-project": OPENCRAFTProjectID } : {}),
              "x-OPENCRAFT-session": input.sessionID,
              "x-OPENCRAFT-request": input.user.id,
              "x-OPENCRAFT-client": flags.client,
              "User-Agent": `OPENCRAFT/${InstallationVersion}`,
            }
          : {
