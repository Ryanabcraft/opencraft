...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@OPENCRAFT-ai/llm"
import type { LLMClientShape } from "@OPENCRAFT-ai/llm/route"
...
export function status(input: Pick<StreamInput, "model" | "provider" | "auth">): RuntimeStatus {
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("OPENCRAFT"))
    return { type: "unsupported", reason: "provider is not openai, OPENCRAFT, or anthropic" }
