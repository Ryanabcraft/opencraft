...
import { PositiveInt } from "@OPENCRAFT-ai/core/schema"
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("TaskStatusTool.execute")(function* (
      ...
      if (!flags.experimentalBackgroundSubagents) {
        return yield* Effect.fail(new Error("task_status requires OPENCRAFT_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true"))
      }
