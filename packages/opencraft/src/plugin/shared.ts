...
export const DEPRECATED_PLUGIN_PACKAGES = ["OPENCRAFT-openai-codex-auth", "OPENCRAFT-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, OPENCRAFTVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(OPENCRAFTVersion) || semver.major(OPENCRAFTVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.OPENCRAFT
  if (typeof range !== "string") return
  if (!semver.satisfies(OPENCRAFTVersion, range)) {
    throw new Error(`Plugin requires OPENCRAFT ${range} but running ${OPENCRAFTVersion}`)
  }
}
