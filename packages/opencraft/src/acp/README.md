...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for OPENCRAFT.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal OPENCRAFT sessions
...
OPENCRAFT acp --cwd /path/to/project
```
...
```bash
OPENCRAFT_ENABLE_QUESTION_TOOL=1 OPENCRAFT acp
```
...
- **Terminal Integration**: Full terminal support via OPENCRAFT's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | OPENCRAFT acp
```
...
### Mapping to OPENCRAFT

ACP sessions map cleanly to OPENCRAFT's internal session model:
