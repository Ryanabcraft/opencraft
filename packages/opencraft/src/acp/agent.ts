...
import type { AssistantMessage, Event, OPENCRAFTClient, SessionMessageResponse, ToolPart } from "@OPENCRAFT-ai/sdk/v2"
...
export function init({ sdk: _sdk }: { sdk: OPENCRAFTClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `OPENCRAFT auth login` in the terminal",
      name: "Login with OPENCRAFT",
      id: "OPENCRAFT-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // OPENCRAFT stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const OPENCRAFTProvider = providers.find((p) => p.id === "OPENCRAFT")
  if (OPENCRAFTProvider) {
    const [best] = Provider.sort(Object.values(OPENCRAFTProvider.models))
    if (best) {
      return {
