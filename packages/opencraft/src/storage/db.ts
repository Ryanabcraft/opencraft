...
import { InstallationChannel } from "@OPENCRAFT-ai/core/installation/version"
import { EffectBridge } from "@/effect/bridge"
import { init } from "#db"
import { Effect, Schema } from "effect"

declare const OPENCRAFT_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "OPENCRAFT.db")
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  return path.join(Global.Path.data, `OPENCRAFT-${safe}.db`)
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.OPENCRAFT_DB) {
    if (Flag.OPENCRAFT_DB === ":memory:" || path.isAbsolute(Flag.OPENCRAFT_DB)) return Flag.OPENCRAFT_DB
    return path.join(Global.Path.data, Flag.OPENCRAFT_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    const entries =
      typeof OPENCRAFT_MIGRATIONS !== "undefined"
        ? OPENCRAFT_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof OPENCRAFT_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
