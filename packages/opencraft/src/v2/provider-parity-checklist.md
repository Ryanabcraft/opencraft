...
This tracks legacy provider behavior from `packages/OPENCRAFT/src/provider/provider.ts` that still needs to be ported into the v2 provider plugins under `packages/OPENCRAFT/src/v2/plugin/provider/`. Keep entries checked only when v2 has equivalent behavior or when the item is intentionally skipped.
...
- [x] OPENCRAFT unauthenticated paid-model filtering.
- [x] OPENCRAFT public API key fallback.
...
- [x] OPENCRAFT small model priority. Replaced by cheapest output cost selection.
