...
export const OPENCRAFT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENCRAFT_MODE_KEY = "OPENCRAFT.mode"

export const OPENCRAFTKeymapProvider = KeymapProvider
export const useOPENCRAFTKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OPENCRAFTModeStack = ReturnType<typeof createOPENCRAFTModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OPENCRAFTModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOPENCRAFTModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENCRAFT_MODE_KEY, OPENCRAFT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENCRAFT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENCRAFT_MODE_KEY, stack.at(-1)?.mode ?? OPENCRAFT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENCRAFT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENCRAFT_MODE_KEY, undefined)
...
export function useOPENCRAFTModeStack() {
  return getOPENCRAFTModeStack(useOPENCRAFTKeymap())
}

export function getOPENCRAFTModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("OPENCRAFT mode stack is not registered for this keymap")
  return value
}
...
export function registerOPENCRAFTKeymap(
  keymap: OpenTuiKeymap,
  renderer: CliRenderer,
  config: Pick<TuiConfig.Resolved, "keybinds" | "leader_timeout">,
) {
  const modeStack = createOPENCRAFTModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useOPENCRAFTKeymap()
