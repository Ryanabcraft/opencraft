...
import { Flock } from "@OPENCRAFT-ai/core/util/flock"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".OPENCRAFT"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".OPENCRAFT", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".OPENCRAFT", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void; disposeTimeoutMs?: number }) {
  ...
  try {
    ...
    const records = Flag.OPENCRAFT_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.OPENCRAFT_PURE && config.plugin_origins?.length) {
