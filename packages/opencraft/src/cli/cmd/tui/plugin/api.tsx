import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@OPENCRAFT-ai/plugin/tui"
...
import type { useOPENCRAFTKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOPENCRAFTKeymap>
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getOPENCRAFTModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOPENCRAFTModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
