...
import type { TuiThemeCurrent } from "@OPENCRAFT-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "OPENCRAFT")
        draft.active = typeof active === "string" ? active : "OPENCRAFT"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.OPENCRAFT, store.mode)
    })
