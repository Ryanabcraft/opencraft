...
const TUI_SCHEMA_URL = "https://OPENCRAFT.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from OPENCRAFT.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const OPENCRAFT = await OPENCRAFTFiles(input)
  for (const file of OPENCRAFT) {
...
async function OPENCRAFTFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "OPENCRAFT"),
    ...(await Filesystem.findUp(["OPENCRAFT.json", "OPENCRAFT.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "OPENCRAFT"))
  }
  if (Flag.OPENCRAFT_CONFIG) files.push(Flag.OPENCRAFT_CONFIG)
