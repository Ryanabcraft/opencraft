...
import { createOPENCRAFTClient } from "@OPENCRAFT-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "OPENCRAFT.mode": input.resolveSession ? "local" : "attach",
      "OPENCRAFT.initial_input": !!input.initialInput,
      "OPENCRAFT.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "OPENCRAFT.directory": ctx.directory,
        "OPENCRAFT.resume": ctx.resume === true,
        "OPENCRAFT.agent.name": state.agent,
        "OPENCRAFT.model.provider": state.model?.providerID,
        "OPENCRAFT.model.id": state.model?.modelID,
        "OPENCRAFT.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "OPENCRAFT.model.provider": model.providerID,
            "OPENCRAFT.model.id": model.modelID,
            "OPENCRAFT.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "OPENCRAFT.model.id": state.model?.modelID,
                "OPENCRAFT.model.variant": state.activeVariant,
                "OPENCRAFT.prompt.chars": prompt.text.length,
                "OPENCRAFT.prompt.parts": prompt.parts.length,
                "OPENCRAFT.prompt.include_files": includeFiles,
                "OPENCRAFT.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "OPENCRAFT.directory": input.directory,
      "OPENCRAFT.initial_input": !!input.initialInput,
      "OPENCRAFT.demo": input.demo,
    },
    async () => {
