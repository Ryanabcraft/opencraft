...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "OPENCRAFT.agent.name": input.agent,
      "OPENCRAFT.directory": input.directory,
      "OPENCRAFT.first": input.first,
      "OPENCRAFT.model.provider": input.model?.providerID,
      "OPENCRAFT.model.id": input.model?.modelID,
      "OPENCRAFT.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "OPENCRAFT.show_exit": next.showExit,
