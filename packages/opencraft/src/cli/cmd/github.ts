...
import { ModelsDev } from "@OPENCRAFT-ai/core/models-dev"
...
const AGENT_USERNAME = "OPENCRAFT-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/OPENCRAFT.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://OPENCRAFT.ai/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/OPENCRAFT-agent"
          const command =
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.OPENCRAFT.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
jobs:
  OPENCRAFT:
    if: |
      ...
      startsWith(github.event.comment.body, '/OPENCRAFT')
    ...
    steps:
      ...
      - name: Run OPENCRAFT
        uses: anomalyco/OPENCRAFT/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.OPENCRAFT.ai" : "https://OPENCRAFT.ai"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.OPENCRAFT.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/OPENCRAFT,/oc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("OPENCRAFT-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `OPENCRAFT/${type}-${hex}-${timestamp}`
        }
        return `OPENCRAFT/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/OPENCRAFT-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[OPENCRAFT session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the OPENCRAFT infrastructure after your response",
