...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall OPENCRAFT and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OPENCRAFT")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g OPENCRAFT-ai",
      pnpm: "pnpm uninstall -g OPENCRAFT-ai",
      bun: "bun remove -g OPENCRAFT-ai",
      yarn: "yarn global remove OPENCRAFT-ai",
      brew: "brew uninstall OPENCRAFT",
      choco: "choco uninstall OPENCRAFT",
      scoop: "scoop uninstall OPENCRAFT",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "OPENCRAFT", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OPENCRAFT!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# OPENCRAFT") || content.includes(".OPENCRAFT/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".OPENCRAFT/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".OPENCRAFT/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".OPENCRAFT"))
    ) {
      continue
    }
