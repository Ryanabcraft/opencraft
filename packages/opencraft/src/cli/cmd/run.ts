// CLI entry point for `OPENCRAFT run`.
//
// Handles three modes:
...
//      OPENCRAFT server and runs interactive mode against it.
//
...
import { createOPENCRAFTClient, type OPENCRAFTClient, type ToolPart } from "@OPENCRAFT-ai/sdk/v2"
...
type ModelInput = Parameters<OPENCRAFTClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run OPENCRAFT with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running OPENCRAFT server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENCRAFT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCRAFT_SERVER_USERNAME or 'OPENCRAFT')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: OPENCRAFTClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OPENCRAFTClient, sessionID: string) {
      ...
      async function current(sdk: OPENCRAFTClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: OPENCRAFTClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OPENCRAFTClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: OPENCRAFTClient) {
        ...
        async function loop(client: OPENCRAFTClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createOPENCRAFTClient({
        baseUrl: "http://OPENCRAFT.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
