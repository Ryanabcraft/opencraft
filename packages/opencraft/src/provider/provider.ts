...
import { Hash } from "@OPENCRAFT-ai/core/util/hash"
import { Plugin } from "../plugin"
import { type LanguageModelV3 } from "@ai-sdk/provider"
import * as ModelsDev from "@OPENCRAFT-ai/core/models-dev"
import { Auth } from "../auth"
import { Env } from "../env"
import { InstallationVersion } from "@OPENCRAFT-ai/core/installation/version"
...
import { optionalOmitUndefined } from "@OPENCRAFT-ai/core/schema"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@OPENCRAFT-ai/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    OPENCRAFT: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["OPENCRAFT"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from OPENCRAFT.json provider config
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://OPENCRAFT.ai/",
            "X-Title": "OPENCRAFT",
            "X-BILLING-INVOKE-ORIGIN": "OPENCRAFT",
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `OPENCRAFT/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `OPENCRAFT/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `OPENCRAFT auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `OPENCRAFT/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "OPENCRAFT",
          },
        },
      }),
    kilo: () =>
...
export class Service extends Context.Service<Service, Interface>()("@OPENCRAFT/Provider") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      if (providerID.startsWith("OPENCRAFT")) {
        priority = ["gpt-5-nano"]
      }
