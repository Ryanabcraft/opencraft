...
import { OPENCRAFTHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for OPENCRAFT serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OPENCRAFTHttpApi)
...
export const PublicApi = OPENCRAFTHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "OPENCRAFT",
    version: "1.0.0",
    description: "OPENCRAFT api",
