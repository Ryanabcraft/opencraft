// Subprocess test harness for the OPENCRAFT CLI. Spawns the real binary against
...
//
// Configuration flows through OPENCRAFT's built-in test affordances:
//   - OPENCRAFT_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCRAFT_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCRAFT_DISABLE_PROJECT_CONFIG : skip walking up for OPENCRAFT.json
//   - OPENCRAFT_PURE                : skip external plugin discovery + install
//   - OPENCRAFT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `OPENCRAFT.run` is fully wired. The shape supports adding more
// builders (`OPENCRAFT.serve(opts)`, `OPENCRAFT.acp(opts)`, `OPENCRAFT.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OPENCRAFTCli.
import { test, type TestOptions } from "bun:test"
import { AppFileSystem } from "@OPENCRAFT-ai/core/filesystem"
import { AppProcess } from "@OPENCRAFT-ai/core/process"
...
const OPENCRAFTRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(OPENCRAFTRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OPENCRAFT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OPENCRAFT_CONFIG_CONTENT: configJson,
    OPENCRAFT_DISABLE_PROJECT_CONFIG: "1",
    OPENCRAFT_PURE: "1",
    OPENCRAFT_DISABLE_AUTOUPDATE: "1",
    OPENCRAFT_DISABLE_AUTOCOMPACT: "1",
    OPENCRAFT_DISABLE_MODELS_FETCH: "1",
    OPENCRAFT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `OPENCRAFT run`. New flags should
...
// `OPENCRAFT serve` is a long-lived process — it never exits on its own.
...
// `OPENCRAFT acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OPENCRAFTCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `OPENCRAFT serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `OPENCRAFT acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly OPENCRAFT: OPENCRAFTCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("OPENCRAFT.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("OPENCRAFT.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "OPENCRAFT server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `OPENCRAFT serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("OPENCRAFT.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const OPENCRAFT: OPENCRAFTCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, OPENCRAFT })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `OPENCRAFT.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "OPENCRAFT") {
...
// every call site. Use this for any test that needs the OPENCRAFT CLI fixture.
//
...
// resources (e.g. `OPENCRAFT.serve`) without an extra `Effect.scoped` wrapper —
