 ...
 *
 * The script intentionally isolates `OPENCRAFT_DB` before importing modules that touch
...
void (await import("@OPENCRAFT-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "OPENCRAFT.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "OPENCRAFT.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-OPENCRAFT-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-OPENCRAFT-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "OPENCRAFT", model: "test" })}`,
      headers: ctx.headers(),
    }))
