...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-OPENCRAFT-directory", directory)
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-OPENCRAFT-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://OPENCRAFT.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-OPENCRAFT-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-OPENCRAFT-workspace")
      } finally {
