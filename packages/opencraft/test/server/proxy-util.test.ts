...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips OPENCRAFT-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-OPENCRAFT-directory": "/home/user/project",
      ...
      const result = ProxyUtil.headers(req)
      expect(result.get("x-OPENCRAFT-directory")).toBeNull()
      expect(result.get("x-OPENCRAFT-workspace")).toBeNull()
