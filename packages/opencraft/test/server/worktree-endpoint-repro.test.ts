...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      OPENCRAFT_EXPERIMENTAL_WORKSPACES: Flag.OPENCRAFT_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.OPENCRAFT_EXPERIMENTAL_WORKSPACES = original.OPENCRAFT_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
