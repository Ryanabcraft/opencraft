...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips OPENCRAFT-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-OPENCRAFT-directory"]).toBeUndefined()
      expect(forwarded["x-OPENCRAFT-workspace"]).toBeUndefined()
