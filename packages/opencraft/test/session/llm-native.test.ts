...
import { ToolFailure } from "@OPENCRAFT-ai/llm"
...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderID.make("OPENCRAFT") },
        provider: { ...providerInfo, id: ProviderID.make("OPENCRAFT") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, OPENCRAFT, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored OPENCRAFT auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderID.make("OPENCRAFT") },
        provider: {
          ...providerInfo,
          id: ProviderID.make("OPENCRAFT"),
