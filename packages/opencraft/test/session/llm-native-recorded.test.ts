...
import { HttpRecorder, Redactor } from "@OPENCRAFT-ai/http-recorder"
...
import { LLMEvent, LLMResponse } from "@OPENCRAFT-ai/llm"
import { LLMClient, RequestExecutor } from "@OPENCRAFT-ai/llm/route"
import { RuntimeFlags } from "@/effect/runtime-flags"
import type { Agent } from "../../src/agent/agent"
import { LLM } from "../../src/session/llm"
import { MessageV2 } from "../../src/session/message-v2"
import { MessageID, SessionID } from "../../src/session/schema"
import type { ModelsDev } from "@OPENCRAFT-ai/core/models-dev"
import { TestInstance } from "../fixture/fixture"
import { testEffect } from "../lib/effect"

const FIXTURES_DIR = path.join(import.meta.dir, "../fixtures/recordings")

const zenURL = (connection: string) => `https://console.OPENCRAFT.ai/proxy/connections/${connection}/v1`
...
const PROVIDERS = {
  openai: {
    ...
    tags: ["OPENCRAFT", "native", "tool-loop"],
    canRecord: Boolean(process.env.OPENCRAFT_RECORD_OPENAI_API_KEY ?? process.env.OPENAI_API_KEY),
    config: (model) => ({
      ...
      provider: {
        openai: {
          ...
          options: {
            apiKey: process.env.OPENCRAFT_RECORD_OPENAI_API_KEY ?? process.env.OPENAI_API_KEY ?? "fixture-openai-key",
  ...
  OPENCRAFT: {
    providerID: ProviderID.OPENCRAFT,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["OPENCRAFT", "zen", "native", "tool-loop"],
    canRecord: Boolean(process.env.OPENCRAFT_RECORD_CONSOLE_TOKEN && process.env.OPENCRAFT_RECORD_ZEN_ORG_ID),
    config: (model) => ({
      enabled_providers: ["OPENCRAFT"],
      provider: {
        OPENCRAFT: {
          name: "OPENCRAFT Zen",
          env: ["OPENCRAFT_CONSOLE_TOKEN"],
          ...
          api: zenURL(process.env.OPENCRAFT_RECORD_ZEN_CONNECTION ?? "fixture"),
          models: { [model.id]: cloneModel(model) },
          options: {
            apiKey: process.env.OPENCRAFT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
            headers: { "x-org-id": process.env.OPENCRAFT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
          },
        },
      },
    }),
  ...
  anthropic: {
    ...
    tags: ["OPENCRAFT", "native", "tool-loop"],
    canRecord: Boolean(process.env.OPENCRAFT_RECORD_ANTHROPIC_API_KEY ?? process.env.ANTHROPIC_API_KEY),
    config: (model) => ({
      ...
      provider: {
        anthropic: {
          ...
          options: {
            apiKey:
              process.env.OPENCRAFT_RECORD_ANTHROPIC_API_KEY ?? process.env.ANTHROPIC_API_KEY ?? "fixture-anthropic-key",
...
function recordedNativeLLMLayer(spec: ProviderSpec) {
  // Only the HTTP client is recorded; RequestExecutor and the OPENCRAFT LLM stack remain real.
...
const writeConfig = (directory: string, spec: ProviderSpec, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "OPENCRAFT.json"),
      JSON.stringify({ $schema: "https://OPENCRAFT.ai/config.json", ...spec.config(model) }),
    ),
  )
