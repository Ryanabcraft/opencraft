...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "OPENCRAFT.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "OPENCRAFT.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "OPENCRAFT.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "OPENCRAFT.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["OPENCRAFT.json", "OPENCRAFT.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "OPENCRAFT.json"),
        path.join(tmp.path, "OPENCRAFT.jsonc"),
        path.join(project, "OPENCRAFT.json"),
        path.join(project, "OPENCRAFT.jsonc"),
      ])
    })
  })
