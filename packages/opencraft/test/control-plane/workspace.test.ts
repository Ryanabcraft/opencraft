...
const originalEnv = {
  OPENCRAFT_AUTH_CONTENT: process.env.OPENCRAFT_AUTH_CONTENT,
  OPENCRAFT_EXPERIMENTAL_WORKSPACES: process.env.OPENCRAFT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  Database.close()
  restoreEnv()
  process.env.OPENCRAFT_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@OPENCRAFT.test"`.cwd(dir).quiet()
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (instance) => {
      process.env.OPENCRAFT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
      process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
      process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=OPENCRAFT-test"
      ...
      expect(JSON.parse(recorded.calls.create[0].env.OPENCRAFT_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.OPENCRAFT_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.OPENCRAFT_EXPERIMENTAL_WORKSPACES).toBe("true")
      ...
      expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=OPENCRAFT-test")
