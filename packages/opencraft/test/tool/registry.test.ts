...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".OPENCRAFT")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .OPENCRAFT/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const OPENCRAFT = path.join(test.directory, ".OPENCRAFT")
      const tool = path.join(OPENCRAFT, "tool")
  ...
  it.instance("ignores non-tool exports in .OPENCRAFT/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".OPENCRAFT", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-OPENCRAFT` was registering a tool
  ...
  it.instance("loads tools from .OPENCRAFT/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const OPENCRAFT = path.join(test.directory, ".OPENCRAFT")
      const tools = path.join(OPENCRAFT, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".OPENCRAFT", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const OPENCRAFT = path.join(test.directory, ".OPENCRAFT")
        const customTools = path.join(OPENCRAFT, "tools")
        const plugin = path.join(OPENCRAFT, "node_modules", "@OPENCRAFT-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(OPENCRAFT, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@OPENCRAFT-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@OPENCRAFT-ai/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".OPENCRAFT", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(OPENCRAFT, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@OPENCRAFT-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(OPENCRAFT, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(OPENCRAFT, "node_modules", "cowsay")
