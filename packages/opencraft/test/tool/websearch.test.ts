...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.OPENCRAFT_WEBSEARCH_PROVIDER

    try {
      process.env.OPENCRAFT_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.OPENCRAFT_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.OPENCRAFT_WEBSEARCH_PROVIDER
      else process.env.OPENCRAFT_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for OPENCRAFT or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderID.OPENCRAFT, { exa: false, parallel: false })).toBe(true)
