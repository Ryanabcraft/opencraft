...
it.instance("plan agent denies edits except .OPENCRAFT/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".OPENCRAFT/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".OPENCRAFT", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENCRAFT_TEST_HOME
      process.env.OPENCRAFT_TEST_HOME = test.directory
