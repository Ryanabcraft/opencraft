...
import { OPENCRAFTClient, type GlobalEvent } from "@OPENCRAFT-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OPENCRAFTClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OPENCRAFTClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OPENCRAFTClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OPENCRAFTClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OPENCRAFTClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://OPENCRAFT.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OPENCRAFTClient["event"]["subscribe"]
    globalEvent?: OPENCRAFTClient["global"]["event"]
    promptAsync?: OPENCRAFTClient["session"]["promptAsync"]
    status?: OPENCRAFTClient["session"]["status"]
    messages?: OPENCRAFTClient["session"]["messages"]
    children?: OPENCRAFTClient["session"]["children"]
    permissions?: OPENCRAFTClient["permission"]["list"]
    questions?: OPENCRAFTClient["question"]["list"]
  } = {},
) {
  const client = new OPENCRAFTClient()

  const subscribe: OPENCRAFTClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OPENCRAFTClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OPENCRAFTClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OPENCRAFTClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OPENCRAFTClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OPENCRAFTClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OPENCRAFTClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OPENCRAFTClient["question"]["list"] = input.questions ?? (() => ok([]))
