...
async function mountPrompt(input: {
  ...
  const { Global } = await import("@OPENCRAFT-ai/core/global")
  const previous = {
  ...
  const [
    ...
    { TuiConfigProvider },
    { ToastProvider },
    { OPENCRAFTKeymapProvider, registerOPENCRAFTKeymap },
  ] = await Promise.all([
  ...
  function Harness() {
    ...
    const off = registerOPENCRAFTKeymap(keymap, renderer, resolvedConfig)
    onCleanup(off)
