...
// terminal sizes. The default OPENCRAFT tui command is excluded —
// `OPENCRAFT --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `OPENCRAFT --help` prints today;
...
// top-level help on `--help` and exits 1; not a real OPENCRAFT command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("OPENCRAFT CLI help-text snapshots", () => {
    ...
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* OPENCRAFT.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`OPENCRAFT ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`OPENCRAFT ${argv.join(" ")} --help`)
        }
