...
describe("OPENCRAFT read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OPENCRAFT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        const r = yield* OPENCRAFT.spawn(["mcp", "list"])
        OPENCRAFT.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        const r = yield* OPENCRAFT.spawn(["providers", "list"])
        OPENCRAFT.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        const r = yield* OPENCRAFT.spawn(["models"])
        OPENCRAFT.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        const r = yield* OPENCRAFT.spawn(["agent", "list"])
        OPENCRAFT.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OPENCRAFT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        const r = yield* OPENCRAFT.spawn(["session", "list"])
        OPENCRAFT.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        const r = yield* OPENCRAFT.spawn(["stats"])
        OPENCRAFT.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OPENCRAFT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ OPENCRAFT }) =>
      Effect.gen(function* () {
        const r = yield* OPENCRAFT.spawn(["db", "path"])
        OPENCRAFT.expectExit(r, 0, "db path")
