...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/OPENCRAFT.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/OPENCRAFT.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/OPENCRAFT.jsonc", dir: ".OPENCRAFT", suggestion: "OPENCRAFT" },
        expected:
          'Directory ".OPENCRAFT" in /tmp/OPENCRAFT.jsonc is not valid. Rename the directory to "OPENCRAFT" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/OPENCRAFT.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/OPENCRAFT.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.OPENCRAFT.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.OPENCRAFT.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `OPENCRAFT models` to list available models",
      "Or check your config (OPENCRAFT.json) provider/model names",
    ].join("\n")
