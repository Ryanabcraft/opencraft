...
import { EffectFlock } from "@OPENCRAFT-ai/core/util/effect-flock"
...
import { AppFileSystem } from "@OPENCRAFT-ai/core/filesystem"
import { Env } from "../../src/env"
import { provideTestInstance, provideTmpdirInstance, TestInstance, withTestInstance } from "../fixture/fixture"
import { tmpdir } from "../fixture/fixture"
import { InstanceRuntime } from "@/project/instance-runtime"
import { CrossSpawnSpawner } from "@OPENCRAFT-ai/core/cross-spawn-spawner"
...
import { Global } from "@OPENCRAFT-ai/core/global"
...
const managedConfigDir = process.env.OPENCRAFT_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "OPENCRAFT.json") {
...
async function writeConfig(dir: string, config: object, name = "OPENCRAFT.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "OPENCRAFT.json") =>
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://OPENCRAFT.ai/config.json",
      snapshot: false,
    })
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "OPENCRAFT.jsonc"))
    expect(content).toContain('"$schema": "https://OPENCRAFT.ai/config.json"')
  } finally {
    ;(Global.Path as { config: string }).config = prev
    await clear(true)
  }
...
test("does not create global config when OPENCRAFT_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.OPENCRAFT_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.OPENCRAFT_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "OPENCRAFT.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.OPENCRAFT_CONFIG_DIR
    else process.env.OPENCRAFT_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://OPENCRAFT.ai/config.json", shell: "bash" },
      "config.json",
    )
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "OPENCRAFT.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "OPENCRAFT.jsonc")
...
it.instance("ignores legacy tui keys in OPENCRAFT config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.promise(() =>
      Filesystem.write(
        path.join(test.directory, "OPENCRAFT.jsonc"),
        `{
        // This is a comment
        "$schema": "https://OPENCRAFT.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Filesystem.write(
          path.join(test.directory, "OPENCRAFT.json"),
          JSON.stringify({
      ...
      const content = yield* Effect.promise(() => Filesystem.readText(path.join(test.directory, "OPENCRAFT.json")))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["OPENCRAFT_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { OPENCRAFT: { options: { apiKey: "{env:OPENCRAFT_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["OPENCRAFT"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["OPENCRAFT_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENCRAFT_CONSOLE_TOKEN"]
    }
  }
})
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* Effect.promise(() => Filesystem.write(path.join(test.directory, "OPENCRAFT.json"), "{ invalid json }"))
    const exit = yield* Config.Service.use((svc) => svc.get()).pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .OPENCRAFT directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".OPENCRAFT", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".OPENCRAFT", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .OPENCRAFT/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".OPENCRAFT", "agents", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .OPENCRAFT/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".OPENCRAFT", "command", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .OPENCRAFT/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".OPENCRAFT", "commands", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
test("does not try to install dependencies in read-only OPENCRAFT_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENCRAFT_CONFIG_DIR
  process.env.OPENCRAFT_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENCRAFT_CONFIG_DIR
    else process.env.OPENCRAFT_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENCRAFT_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .OPENCRAFT config
      const projectDir = path.join(dir, "project")
      const OPENCRAFTDir = path.join(projectDir, ".OPENCRAFT")
      await fs.mkdir(OPENCRAFTDir, { recursive: true })

      // Global config with plugins
      await Filesystem.write(
        path.join(dir, "OPENCRAFT.json"),
        JSON.stringify({
          $schema: "https://OPENCRAFT.ai/config.json",
      ...
      // Local .OPENCRAFT config with different plugins
      await Filesystem.write(
        path.join(OPENCRAFTDir, "OPENCRAFT.json"),
        JSON.stringify({
          $schema: "https://OPENCRAFT.ai/config.json",
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".OPENCRAFT", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".OPENCRAFT", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .OPENCRAFT config with some overlapping plugins
      await Filesystem.write(
        path.join(OPENCRAFTDir, "OPENCRAFT.json"),
        JSON.stringify({
          $schema: "https://OPENCRAFT.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".OPENCRAFT")
      ...
      await Filesystem.write(
        path.join(local, "OPENCRAFT.json"),
        JSON.stringify({
          $schema: "https://OPENCRAFT.ai/config.json",
...
// Note: preload.ts sets OPENCRAFT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("local .OPENCRAFT config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://OPENCRAFT.ai/config.json",
    ...
    yield* mkdirEffect(path.join(test.directory, ".OPENCRAFT"))
    yield* writeConfigEffect(
      path.join(test.directory, ".OPENCRAFT"),
      {
        $schema: "https://OPENCRAFT.ai/config.json",
        mcp: {
          docs: {
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/OPENCRAFT")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/OPENCRAFT")
    ...
    if (urlStr.includes(".well-known/OPENCRAFT")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/OPENCRAFT.json",
              headers: {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/OPENCRAFT")
            expect(remoteFetchedUrl).toBe("https://config.example.com/OPENCRAFT.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "OPENCRAFT.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-OPENCRAFT@2.4.3", file)).toBe("oh-my-OPENCRAFT@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-OPENCRAFT@2.4.3", "file:///project/.OPENCRAFT/plugin/oh-my-OPENCRAFT.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.OPENCRAFT/plugin/demo.ts", "file:///project/.OPENCRAFT/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.OPENCRAFT/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const OPENCRAFTDir = path.join(projectDir, ".OPENCRAFT")
        const pluginDir = path.join(OPENCRAFTDir, "plugin")
...
describe("OPENCRAFT_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .OPENCRAFT/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .OPENCRAFT directory with a command
          const OPENCRAFTDir = path.join(dir, ".OPENCRAFT", "command")
          await fs.mkdir(OPENCRAFTDir, { recursive: true })
          await Filesystem.write(path.join(OPENCRAFTDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await withTestInstance({
        directory: tmp.path,
        fn: async (ctx) => {
          ...
          // Project .OPENCRAFT should NOT be in directories list
          const hasProjectOPENCRAFT = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectOPENCRAFT).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENCRAFT_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENCRAFT_CONFIG_DIR"]
      process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENCRAFT_CONFIG_DIR"]
      } else {
        process.env["OPENCRAFT_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENCRAFT_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENCRAFT_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENCRAFT_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENCRAFT_CONFIG_DIR"] = configDirTmp.path

      await withTestInstance({
        directory: projectTmp.path,
        fn: async (ctx) => {
          const config = await load(ctx)
          // Should load from OPENCRAFT_CONFIG_DIR, not project
...
// Regression for #28206: malformed OPENCRAFT_PERMISSION JSON used to crash
...
describe("OPENCRAFT_PERMISSION env var", () => {
  test("does not crash when OPENCRAFT_PERMISSION contains invalid JSON", async () => {
    const original = process.env["OPENCRAFT_PERMISSION"]
    process.env["OPENCRAFT_PERMISSION"] = "{invalid"
    try {
      await using tmp = await tmpdir()
      await withTestInstance({
        directory: tmp.path,
      ...
      if (original !== undefined) {
        process.env["OPENCRAFT_PERMISSION"] = original
      } else {
        delete process.env["OPENCRAFT_PERMISSION"]
      }
    }
  })
})

describe("OPENCRAFT_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENCRAFT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENCRAFT_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENCRAFT_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://OPENCRAFT.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENCRAFT_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENCRAFT_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENCRAFT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENCRAFT_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "OPENCRAFT Managed",
          PayloadIdentifier: "ai.OPENCRAFT.managed.test",
          PayloadType: "ai.OPENCRAFT.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://OPENCRAFT.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://OPENCRAFT.ai/config.json")
})
