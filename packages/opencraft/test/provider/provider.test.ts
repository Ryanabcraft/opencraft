...
function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("OPENCRAFT")]
...
test("ModelNotFoundError suggests catalog models for unloaded providers", async () => {
  ...
  await withTestInstance({
    directory: tmp.path,
    fn: async (ctx) => {
      await remove(ctx, "OPENCRAFT_API_KEY")
      try {
        await getModel(ProviderID.OPENCRAFT, ModelID.make("claude-haiku-fake-model"), ctx)
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "OPENCRAFT" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".OPENCRAFT")
...
test("OPENCRAFT loader keeps paid models when config apiKey is present", async () => {
...
test("OPENCRAFT loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "OPENCRAFT.json"),
