    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".OPENCRAFT/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const OPENCRAFTModel = {
      ...openaiModel,
      providerID: "OPENCRAFT",
      api: {
        id: "OPENCRAFT-test",
        url: "https://api.OPENCRAFT.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, OPENCRAFTModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.OPENCRAFT?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.OPENCRAFT?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          OPENCRAFT: { itemId: "msg_OPENCRAFT" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              OPENCRAFT: { itemId: "msg_OPENCRAFT_part" },
    ...
    expect(result[0].providerOptions?.OPENCRAFT?.itemId).toBe("msg_OPENCRAFT")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.OPENCRAFT?.itemId).toBe("msg_OPENCRAFT_part")
