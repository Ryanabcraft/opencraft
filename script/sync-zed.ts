...
const EXTENSION_NAME = "OPENCRAFT"
...
async function main() {
  ...
  // Delete any existing branches for OPENCRAFT updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating OPENCRAFT extension to v${cleanVersion}"`
