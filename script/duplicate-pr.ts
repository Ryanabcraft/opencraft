...
import { createOPENCRAFT } from "@OPENCRAFT-ai/sdk"
...
`)
  ...
  const OPENCRAFT = await createOPENCRAFT({ port: 0 })
  ...
  try {
    ...
    const session = await OPENCRAFT.client.session.create()
    const result = await OPENCRAFT.client.session
      .prompt({
