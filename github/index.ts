...
import { createOPENCRAFTClient } from "@OPENCRAFT-ai/sdk"
...
const { client, server } = createOPENCRAFT()
...
try {
  ...
  await assertOPENCRAFTConnected()
  ...
  console.log("OPENCRAFT session", session.id)
  if (shareId) {
...
function createOPENCRAFT() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`OPENCRAFT`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createOPENCRAFTClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/OPENCRAFT|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/OPENCRAFT` or `/oc`")
  }
}
...
async function assertOPENCRAFTConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to OPENCRAFT server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.OPENCRAFT.ai" : "https://OPENCRAFT.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.OPENCRAFT.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("OPENCRAFT-github-action")
    response = await fetch("https://api.OPENCRAFT.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/OPENCRAFT" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/OPENCRAFT") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to OPENCRAFT...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "OPENCRAFT-agent[bot]"`
  await $`git config --global user.email "OPENCRAFT-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `OPENCRAFT/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/OPENCRAFT-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[OPENCRAFT session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
