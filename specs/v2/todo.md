...
The OPENCRAFT server has moved to the Effect HttpApi backend. Remaining work is
...
- plugins get global "OPENCRAFT" instance like in that post i showed
- OPENCRAFT instance has stuff like `OPENCRAFT.session.prompt()` or
  `OPENCRAFT.tool.register({...})`
